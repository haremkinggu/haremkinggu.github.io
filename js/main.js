jQuery(document).ready(function(){
	var intro = $('.cd-intro-block'),
		...
		singleProjectContent = $('#one-con'),
		secondProjectContent = $('#two-con'),
		thirdProjectContent = $('#three-con'),
	...
	projectsContainer.on('click', '.cd-slider #one', function(event) {
		var mq = checkMQ();
		event.preventDefault();
	...
	singleProjectContent.on('click', '.close', function(event){
		...
		//select a single project - open project-content panel
	projectsContainer.on('click', '.cd-slider #two', function(event) {
		var mq = checkMQ();
		event.preventDefault();
		if( $(this).parent('li').next('li').is('.current') && (mq == 'desktop') ) {
			prevSides(projectsSlider);
			...
			secondProjectContent.addClass('is-visible');
		}
	});

	//close single project content
	secondProjectContent.on('click', '.close', function(event){
		event.preventDefault();
		secondProjectContent.removeClass('is-visible');
	});
	
			//select a single project - open project-content panel
	projectsContainer.on('click', '.cd-slider #three', function(event) {
		var mq = checkMQ();
		event.preventDefault();
		if( $(this).parent('li').next('li').is('.current') && (mq == 'desktop') ) {
			prevSides(projectsSlider);
			...
			thirdProjectContent.addClass('is-visible');
		}
	});

	//close single project content
	thirdProjectContent.on('click', '.close', function(event){
		event.preventDefault();
		thirdProjectContent.removeClass('is-visible');
	});
